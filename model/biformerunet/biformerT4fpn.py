...
# from code.duibishiyan.biformerunet.bra_legacy import BiLevelRoutingAttention
# from code.duibishiyan.biformerunet._common import Attention, AttentionLePE, DWConv
...
class Unet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.upsample2=F.interpolate(scale_factor=2,mode='bilinear')
        self.upsample4=F.interpolate(scale_factor=4,mode='bilinear')
        self.upsample05=F.interpolate(scale_factor=0.5,mode='bilinear')
        self.upsample025=F.interpolate(scale_factor=0.25,mode='bilinear')
    ...
    def forward(self, inputs):
        ...
        feat2=feat2+self.upsample2(self.conv21s(feat3))+self.upsample4(self.conv22s(feat4))
        feat3=feat3+self.upsample2(self.conv32s(feat4))+self.upsample05(self.conv31s(feat2))
        feat4=feat4+self.upsample2(self.conv42s(feat5))+self.upsample05(self.conv41s(feat3))
        feat5=feat5+self.upsample05(self.conv51s(feat4))+self.upsample025(self.conv52s(feat3))
